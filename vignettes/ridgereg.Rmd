...
This vignette demonstrates how to use the **`ridgereg()`** function (implemented in this package) for predictive modeling with the **BostonHousing** dataset from the `mlbench` package. We compare three regression approaches and provide comprehensive model diagnostics and visualization.
...
cat("Stepwise Regression RMSE:", round(rmse_step, 3), "\n")
```
...
cat("Ridge Regression RMSE:", round(rmse_ridge, 3), "\n")
```
...
# Visual comparison
ggplot(compare_df, aes(x = Model, y = RMSE, fill = Model)) +
